...
/**
 * Superclass of all synchronizer requests. It can be parameterized with the
 * type of the expected result and holds a reference to the synchronizer adapter
 * on which this request works (usually also the adapter that sent the request).
 * Furthermore, it sends this adapter's UUID and receives the remote adapter's
 * UUID before sending or receiving any additional data.
 * 
 * Subclasses must override doRequesting() to send data, and doConfirming() to
...
public abstract class SynchronizerRequestWithConfirmation<RESULT> extends RequestWithConfirmation<RESULT>
{
	...
	public SynchronizerRequestWithConfirmation(SignalProtocol<?> protocol, Enum<?> literal, SynchronizerAdapter synchronizerAdapter)
	{
	...
	protected final RESULT confirming(ExtendedDataInputStream in) throws Exception
	{
		...
		this.logger.info("confirming(), sender: " + this.remoteAdapterUUID);
