...
import org.storydriven.storydiagrams.diagram.custom.dialogs.ReferenceEParameterDialog;
...
public class ActivityParametersSection extends AbstractSection {
	...
	private Button outReferenceButton;

	private ReferenceEParameterDialog referenceDialog;
	...
	private void configure(final boolean isCreating, final boolean isOutgoing, final EParameter eParameter) {
		...
		if (result == Window.OK) {
			...
			Command feature = new RecordingCommand(getEditingDomain()) {
				@Override
			...
			checkButtonStates(isOutgoing);
		}
	}

	private void reference() {
		Collection<EParameter> eParameters = new ArrayList<EParameter>();
		for (EParameter inParameter : getElement().getInParameters()) {
			if (!getElement().getOutParameters().contains(inParameter)) {
				eParameters.add(inParameter);
			}
		}
		referenceDialog.setEParameters(eParameters);
		int result = referenceDialog.open();
		if (result == Window.OK) {
			final EParameter eParameter = referenceDialog.getEParameter();

			Command feature = new RecordingCommand(getEditingDomain()) {
				@Override
				public void doExecute() {
					getElement().getOutParameters().add(eParameter);
				}
			};
			execute(feature);
			outParametersViewer.refresh();
			...
			public void doExecute() {
				for (Object element : selection.toArray()) {
					if (element instanceof EParameter) {
						EObject eParameter = (EParameter) element;
						if (isOutgoing) {
							if (getElement().getInParameters().contains(eParameter)) {
								getElement().getOutParameters().remove(eParameter);
							} else {
								EcoreUtil.delete(eParameter);
							}
						} else {
							if (getElement().getOutParameters().contains(eParameter)) {
								getElement().getInParameters().remove(eParameter);
							} else {
								EcoreUtil.delete(eParameter);
							}
						}
	...
	private void checkButtonStates(boolean isOutgoing) {
		if (isOutgoing) {
		...
		// check if any in parameter could be referenced
		...
		outReferenceButton.setEnabled(canReference);
	}
	...
	protected void hookWidgetListeners() {
		...
		referenceDialog = new ReferenceEParameterDialog(shell);
	...
	private void hookOutListeners() {
		...
		// reference parameter
		outReferenceButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				reference();
			}
	...
	@Override
	protected void createWidgets(Composite parent, TabbedPropertySheetWidgetFactory wf) {
		// input group
		Group inParametersGroup = wf.createGroup(parent, "In Parameters");
		GridLayoutFactory.fillDefaults().margins(6, 6).numColumns(2).applyTo(inParametersGroup);

		// add, remove, configure controls
		Composite inControlsComposite = wf.createFlatFormComposite(inParametersGroup);
		...
		wf.createFlatFormComposite(inParametersGroup);

		inAddButton = wf.createButton(inControlsComposite, EMPTY, SWT.PUSH);
		inAddButton.setImage(DiagramImages.getImage(DiagramImages.CONTROL_ADD));
		inAddButton.setToolTipText("Add In Parameter");

		inRemoveButton = wf.createButton(inControlsComposite, EMPTY, SWT.PUSH);
		inRemoveButton.setImage(DiagramImages.getImage(DiagramImages.CONTROL_REMOVE));
		inRemoveButton.setToolTipText("Remove Selected In Parameter");
		inRemoveButton.setEnabled(false);

		inConfigureButton = wf.createButton(inControlsComposite, EMPTY, SWT.PUSH);
		...
		// input table
		Table inParametersTable = wf.createTable(inParametersGroup, SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION);
		...
		// up, down controls
		Composite inUpDownControlsComposite = wf.createFlatFormComposite(inParametersGroup);
		GridLayoutFactory.fillDefaults().spacing(0, 6).margins(0, 0).applyTo(inUpDownControlsComposite);

		inUpButton = wf.createButton(inUpDownControlsComposite, EMPTY, SWT.PUSH);
		...
		inDownButton = wf.createButton(inUpDownControlsComposite, "", SWT.PUSH);
		...
		// output group
		Group outParametersGroup = wf.createGroup(parent, "Out Parameters");
		GridLayoutFactory.fillDefaults().margins(6, 6).numColumns(2).applyTo(outParametersGroup);

		// add, remove, configure controls
		Composite outControlsComposite = wf.createFlatFormComposite(outParametersGroup);
		...
		wf.createFlatFormComposite(outParametersGroup);

		outAddButton = wf.createButton(outControlsComposite, EMPTY, SWT.PUSH);
		outAddButton.setImage(DiagramImages.getImage(DiagramImages.CONTROL_ADD));
		outAddButton.setToolTipText("Add Out Parameter");

		outReferenceButton = wf.createButton(outControlsComposite, EMPTY, SWT.PUSH);
		outReferenceButton.setImage(DiagramImages.getImage(DiagramImages.CONTROL_SHORTCUT));
		outReferenceButton.setToolTipText("Reference In Parameter");

		outRemoveButton = wf.createButton(outControlsComposite, EMPTY, SWT.PUSH);
		outRemoveButton.setImage(DiagramImages.getImage(DiagramImages.CONTROL_REMOVE));
		outRemoveButton.setToolTipText("Remove Selected Out Parameter");
		outRemoveButton.setEnabled(false);

		outConfigureButton = wf.createButton(outControlsComposite, EMPTY, SWT.PUSH);
		...
		// output table
		Table outParametersTable = wf.createTable(outParametersGroup, SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION);
		...
		// up, down controls
		Composite outUpDownControlsComposite = wf.createFlatFormComposite(outParametersGroup);
		GridLayoutFactory.fillDefaults().spacing(0, 6).margins(0, 0).applyTo(outUpDownControlsComposite);

		outUpButton = wf.createButton(outUpDownControlsComposite, EMPTY, SWT.PUSH);
		...
		outDownButton = wf.createButton(outUpDownControlsComposite, EMPTY, SWT.PUSH);
