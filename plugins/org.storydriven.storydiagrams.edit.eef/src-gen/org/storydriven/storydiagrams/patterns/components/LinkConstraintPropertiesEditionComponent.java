	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(PatternsViewsRepository.LinkConstraint.Properties.constraintType)) {
				basePart.initConstraintType(EEFUtils.choiceOfValues(linkConstraint, PatternsPackage.eINSTANCE.getLinkConstraint_ConstraintType()), linkConstraint.getConstraintType());
			}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (PatternsPackage.eINSTANCE.getLinkConstraint_ConstraintType().equals(msg.getFeature()) && isAccessible(PatternsViewsRepository.LinkConstraint.Properties.constraintType))
				basePart.setConstraintType((LinkConstraintType)msg.getNewValue());
			...
			try {
				if (PatternsViewsRepository.LinkConstraint.Properties.index == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getLinkConstraint_Index().getEAttributeType(), (String)newValue);
					}
				...
				if (PatternsViewsRepository.LinkConstraint.Properties.constraintType == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getLinkConstraint_ConstraintType().getEAttributeType(), (String)newValue);
					}
				...
				if (PatternsViewsRepository.LinkConstraint.Properties.negative == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getLinkConstraint_Negative().getEAttributeType(), (String)newValue);
					}
