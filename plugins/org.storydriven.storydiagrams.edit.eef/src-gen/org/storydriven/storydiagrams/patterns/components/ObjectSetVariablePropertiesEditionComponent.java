...
public class ObjectSetVariablePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(PatternsViewsRepository.ObjectSetVariable.Properties.bindingState)) {
				basePart.initBindingState(EEFUtils.choiceOfValues(objectSetVariable, PatternsPackage.eINSTANCE.getAbstractVariable_BindingState()), objectSetVariable.getBindingState());
			}
			...
			if (isAccessible(PatternsViewsRepository.ObjectSetVariable.Properties.bindingSemantics)) {
				basePart.initBindingSemantics(EEFUtils.choiceOfValues(objectSetVariable, PatternsPackage.eINSTANCE.getObjectVariable_BindingSemantics()), objectSetVariable.getBindingSemantics());
			}
			if (isAccessible(PatternsViewsRepository.ObjectSetVariable.Properties.bindingOperator)) {
				basePart.initBindingOperator(EEFUtils.choiceOfValues(objectSetVariable, PatternsPackage.eINSTANCE.getObjectVariable_BindingOperator()), objectSetVariable.getBindingOperator());
			}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(PatternsViewsRepository.ObjectSetVariable.Properties.name)) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (PatternsViewsRepository.ObjectSetVariable.Properties.bindingState == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getAbstractVariable_BindingState().getEAttributeType(), (String)newValue);
					}
				...
				if (PatternsViewsRepository.ObjectSetVariable.Properties.bindingSemantics == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getObjectVariable_BindingSemantics().getEAttributeType(), (String)newValue);
					}
				...
				if (PatternsViewsRepository.ObjectSetVariable.Properties.bindingOperator == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(PatternsPackage.eINSTANCE.getObjectVariable_BindingOperator().getEAttributeType(), (String)newValue);
					}
