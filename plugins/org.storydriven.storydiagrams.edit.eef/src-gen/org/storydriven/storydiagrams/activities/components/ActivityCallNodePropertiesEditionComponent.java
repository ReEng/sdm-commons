...
public class ActivityCallNodePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (ActivitiesViewsRepository.ActivityCallNode.Properties.owningActivity == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				owningActivitySettings.setToReference(event.getNewValue());
		...
		if (ActivitiesViewsRepository.ActivityCallNode.Properties.owningActivityNode == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				owningActivityNodeSettings.setToReference(event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
		...
		if (ActivitiesViewsRepository.ActivityCallNode.Properties.callee == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				calleeSettings.setToReference(event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.ActivityCallNode.Properties.comment)) {
			...
			if (CorePackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.ActivityCallNode.Properties.name)) {
				if (msg.getNewValue() != null) {
