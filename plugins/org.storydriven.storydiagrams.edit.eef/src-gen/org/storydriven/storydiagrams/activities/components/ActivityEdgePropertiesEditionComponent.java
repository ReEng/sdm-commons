	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(ActivitiesViewsRepository.ActivityEdge.Properties.guard)) {
				basePart.initGuard(EEFUtils.choiceOfValues(activityEdge, ActivitiesPackage.eINSTANCE.getActivityEdge_Guard()), activityEdge.getGuard());
			}
			// init filters
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (ActivitiesPackage.eINSTANCE.getActivityEdge_Guard().equals(msg.getFeature()) && isAccessible(ActivitiesViewsRepository.ActivityEdge.Properties.guard))
				basePart.setGuard((EdgeGuard)msg.getNewValue());
			...
			try {
				if (ActivitiesViewsRepository.ActivityEdge.Properties.guard == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ActivitiesPackage.eINSTANCE.getActivityEdge_Guard().getEAttributeType(), (String)newValue);
					}
