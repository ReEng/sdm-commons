...
public class StopNodePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.StopNode.Properties.comment)) {
			...
			if (CorePackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.StopNode.Properties.name)) {
				if (msg.getNewValue() != null) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (ActivitiesViewsRepository.StopNode.Properties.flowStopOnly == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ActivitiesPackage.eINSTANCE.getStopNode_FlowStopOnly().getEAttributeType(), (String)newValue);
					}
