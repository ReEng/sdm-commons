...
public class ModifyingStoryNodePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.ModifyingStoryNode.Properties.comment)) {
			...
			if (CorePackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(ActivitiesViewsRepository.ModifyingStoryNode.Properties.name)) {
				if (msg.getNewValue() != null) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (ActivitiesViewsRepository.ModifyingStoryNode.Properties.forEach == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ActivitiesPackage.eINSTANCE.getStoryNode_ForEach().getEAttributeType(), (String)newValue);
					}
