...
/**
 * Superclass of all SynchronizerIndications of the synchronization protocol. It
 * already holds a reference to the synchronizer adapter. It also receives the
 * remote adapter's UUID (can be read using getRemoteAdapterUUID()) and sends
 * this synchronizer adapter's UUID before receiving or sending additional data.
 * 
 * Subclasses must override doIndicating() to receive data, and doResponding()
...
public abstract class SynchronizerIndicationWithResponse extends IndicationWithResponse
{
	...
	public SynchronizerIndicationWithResponse(SignalProtocol<?> protocol, Enum<?> literal, SynchronizerAdapter synchronizerAdapter)
	{
	...
	protected final void indicating(ExtendedDataInputStream in) throws Exception
	{
		...
		this.logger.info("indicating(), sender: " + this.remoteAdapterUUID);
	...
	{
		this.logger.info("responding(), receiver: " + this.remoteAdapterUUID);
