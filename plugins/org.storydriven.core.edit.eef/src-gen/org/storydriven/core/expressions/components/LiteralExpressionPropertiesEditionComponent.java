...
public class LiteralExpressionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(ExpressionsViewsRepository.LiteralExpression.Properties.comment)) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (ExpressionsViewsRepository.LiteralExpression.Properties.value == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ExpressionsPackage.eINSTANCE.getLiteralExpression_Value().getEAttributeType(), (String)newValue);
					}
