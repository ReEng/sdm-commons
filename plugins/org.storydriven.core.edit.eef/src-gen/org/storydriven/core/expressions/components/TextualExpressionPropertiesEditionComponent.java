...
public class TextualExpressionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (CorePackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(ExpressionsViewsRepository.TextualExpression.Properties.comment)) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (ExpressionsViewsRepository.TextualExpression.Properties.expressionText == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ExpressionsPackage.eINSTANCE.getTextualExpression_ExpressionText().getEAttributeType(), (String)newValue);
					}
				...
				if (ExpressionsViewsRepository.TextualExpression.Properties.language == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ExpressionsPackage.eINSTANCE.getTextualExpression_Language().getEAttributeType(), (String)newValue);
					}
				...
				if (ExpressionsViewsRepository.TextualExpression.Properties.languageVersion == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(ExpressionsPackage.eINSTANCE.getTextualExpression_LanguageVersion().getEAttributeType(), (String)newValue);
					}
