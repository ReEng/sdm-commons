...
public class EmbeddedXtextEditor
{
   ...
   private final class SourceViewerFocusListener implements FocusListener
   {
      ...
      public void focusGained(FocusEvent e)
      {
         IContextService contextService = (IContextService) PlatformUI
               ...
               .getActiveEditor().getSite().getService(IContextService.class);
         ...
         {
            fHandlerActivations.add(handlerService
                  .activateHandler(actionHandler.getAction().getId(),
                        actionHandler, fExpression));
         }
      }
   ...
   private static boolean equals(EObject expected, EObject actual)
   {
      ...
      // builder().put(MatchOptions.OPTION_IGNORE_XMI_ID, Boolean.TRUE).build();
    ...
    * @param job the synchronization job that will be scheduled/rescheduled at each modification of
    *           the editor text. It may be use to reconcile the content of the editor with something
    *           else.
   ...
   /**
    * Creates a new EmbeddedXtextEditor. It must have the SWT.V_SCROLL style at least not to throw
    * NPE when computing overview ruler.
    * 
    * @param control the parent composite that will contain the editor
    * @param injector the Guice injector to get Xtext configuration elements
    * @param job the synchronization job that will be scheduled/rescheduled at each modification of
                     ...
                     IProgressMonitor monitor)
               {
                  if (annotationIssueProcessor == null)
                  {
                     annotationIssueProcessor = new AnnotationIssueProcessor(
                           fDocument, fSourceViewer.getAnnotationModel(),
   ...
   /**
    * Updates the text of this editor with the given String or the serialized form of the EObject if
    * the semantic model of the String does not contain any error and is different from the given
    * EObject.
    * 
    * @param eObject
    * @param asString
      ...
      if (eObject != null)
      {
         ...
         if (!asStringResource.getErrors().isEmpty()
               || (asStringResource.getParseResult() != null && !asStringResource
                     .getParseResult().getParseErrors().isEmpty()))
         {
         ...
         else if (asStringEObject != null)
         {
            try
            {
               EmbeddedXtextResource copyResource = (EmbeddedXtextResource) fEmbeddedXtextResourceProvider
                     .get();
               copyResource.setURI(URI.createURI("copyResource."
                     + fFileExtension));
               ...
               try
               {
                  ...
                  {
                     // FIXME: should update with the serialized form of the copyEObject but throw
                     // RuntimeException!!!
                     update(asString);
                  }
                  else
                  {
                     // if there is no error and the content are equals, then we also update with
                     // the string
                     update(asString);
                  }
               }
         ...
         asStringResource.getResourceSet().getResources()
               .remove(asStringResource);
      }
   }
