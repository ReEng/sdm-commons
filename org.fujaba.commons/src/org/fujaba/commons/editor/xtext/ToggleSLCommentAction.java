...
public final class ToggleSLCommentAction extends Action implements IUpdate {
	...
	public void run() {
		...
		if (!fOperationTarget.canDoOperation(operationCode)) {
			if (shell != null)
				MessageDialog.openError(shell, "An error occured", "ToggleComment_error_message=An error occurred while toggling comments."); //$NON-NLS-2$
			return;
		}
	...
	private boolean isSelectionCommented(ISelection selection) {
		...
		try {
			...
			// should not happen
			FujabaCommonsPlugin.getDefault().logError(x.getMessage(), x);
		}
