...
import org.storydriven.modeling.diagram.edit.policies.MatchingPatternItemSemanticEditPolicy;
...
public class MatchingPatternEditPart extends ShapeNodeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof MatchingPatternStoryPatternCompartementEditPart) {
			...
			pane.add(((MatchingPatternStoryPatternCompartementEditPart) childEditPart)
					.getFigure());
			return true;
		}
		return false;
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof MatchingPatternStoryPatternCompartementEditPart) {
			IFigure pane = getPrimaryShape()
					.getFigureStoryPatternInnerRectangle();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.remove(((MatchingPatternStoryPatternCompartementEditPart) childEditPart)
					.getFigure());
			return true;
		}
		return false;
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == SDMElementTypes.ObjectVariable_3007) {
