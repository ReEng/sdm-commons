...
When the matching of a story pattern is successful, the specified modifications are always executed. If the matching step fails, no modifications are executed and thus the story patterns has no side effects.
...
A link variable is typed over one or two corresponding \fe{EReferences} that
conceptually represent a uni-directional (only one \fe{EReference}) or a
bi-directional (two \fe{EReferences}) association in the underlying type graph.

Figure~\ref{fig:associationBetweenObjects} a) shows an exemplary type graph
with an association between the classes \fe{Method} and \fe{GASTType}.
Technically, this association corresponds to two \fe{EReferences} \fe{methods}
and \fe{parameterTypes}.
Figure~\ref{fig:associationBetweenObjects} b) shows a story pattern with a link
variable typed via this association.

\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.9]{figures/AssociationBetweenObjects}
  \caption{An association between objects}
  \label{fig:associationBetweenObjects}
\end{figure}

If two object variables are connected via two link variables that refer to the
same reference, the target object in the underlying model has to be contained
twice in the source model. 
However, this is not possible for unique references.

An example for this is shown in Figure~\ref{fig:twoLinksBetweenObjects}.
The upper part of the figure shows an excerpt of a type graph and the lower part
shows an exemplary story pattern with two object variables and two link variable
between them. The semantics is that a method \fe{method} has two parameters of
the same type \fe{string}. 
The same holds for bidirectional link variables as depicted in
Figure~\ref{fig:associationBetweenObjects} c). 


\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.8]{figures/TwoLinksBetweenObjects}
  \caption{Two link variables between objects}
  \label{fig:twoLinksBetweenObjects}
\end{figure}
...
First, object constraints can be specified to restrict the attribute values to a certain range, thereby restricting the possible matches of a story pattern. 
Second, attribute values can be changed during the graph rewriting step after a successful matching.

We use \emph{object constraints} to restrict the matching of object variables to objects of the instance model that have specific attribute values. 
Thus, object constraints are considered to be part of the LHS and do not change the instance model. 
The object constraints of an object variable are checked directly after matching the object variable. 
...
The values of attributes that are not restricted by an object constraint are not considered during the matching. 
Thus, they may have an arbitrary value. 
In the current version of story patterns, object constraints need to be specified using OCL~\cite{OCL}. 
...
Besides object constraints, \emph{attribute assignments} can be used to change the value of an attribute during the application of a story pattern. 
...
Story patterns also enable to use both, an object constraint and an attributed assignment for the same attribute inside one object variable. Then, the object constraint is used during the matching step. The attribute value of the object matched to the corresponding object variable is then changed as specified by the attribute assignment while enforcing the RHS.

\begin{figure}[htbp]
  \centering
  \includegraphics[scale=1]{figures/AttributeConstraintAndAssignment}
  \caption{Using an Object Constraint and an Attribute Assignment on the Same Attribute}
...
The OCL statements we allow for object constraints and attribute assignments must not traverse the references of the object variables.
...
Figure~\ref{fig:patternConstraint} gives an example for the concrete syntax of a pattern constraint. The story pattern has two bound object variables \fe{class} and \fe{method} of types \fe{GASTClass} and \fe{Method}, respectively. The story pattern matches all methods of \fe{class} in the object set \fe{classMethods}. The pattern constraint is visualized as a label containing the OCL constraint. In the example, the OCL constraint specifies that all \fe{classMethods} need to have a name which is different from the name of \fe{method}. The matching of the story pattern is successful only if the pattern constraint is fulfilled.
