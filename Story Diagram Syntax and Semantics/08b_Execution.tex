...
%
%The overall interpretation of a story diagram is a simple graph traversal.
%The interpretation starts at the story diagram's \emph{StartNode}. 
%The interpreter traverses the control flow graph of the story diagram until it reaches a \emph{StopNode}. 
%All activity nodes are executed by specialized methods, e.g., for \emph{StoryNodes}, \emph{JunctionNodes}, or \emph{ActivityCallNodes}.
%These methods take care of executing that kind of activity node, e.g., invoking the \emph{StoryPatternMatcher}, calling the story diagram interpreter for another story diagram, or evaluating expressions of \emph{JunctionNodes}.
%After that, these methods return the next activity node to execute.
%If a \emph{StopNode} is reached, the return value expressions of all outgoing parameters are evaluated and put into a map, which is returned by the interpreter.
%This maps the parameter names to their values.\footnote{For backward compatibility, all variables of the story diagram are currently returned.}
