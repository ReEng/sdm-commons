...
In the area of reengineering, Niere et al.\ \cite{NSW+02} had the idea to specify design patterns with a graphical DSL which had strong relations to story patterns. In order to detect the specified patterns in source code, these DSL patterns were translated into story diagrams which were then executed through code generation. Later this approach was implemented in the Reclipse Tool Suite \cite{DMT10}. In follow-up work by Fockel \cite{Foc10}, the generated story diagrams were no longer transformed into executable code but interpreted to allow for easier debugging of the pattern specifications.
...
Meyer \cite{Mey09} adds a few special constructs to story diagrams thereby extending them to transformation diagrams. Some of these extensions, such as multiple out parameters, have been integrated into the story diagrams presented in this report. Meyer uses the transformation diagrams to specify refactorings of object-oriented source code. To verify that certain properties of the code (e.g.\ variable accesses) are preserved by the refactorings, he extends Schilling's approach \cite{Sch06} to proving inductive invariants.
