...
For that purpose \emph{story diagrams} have been developed.
...
A story pattern specifies a graph transformation.
...
The identification of the subgraph for modification is called graph matching and includes the subgraph isomorphism problem.

In case of story patterns, the host graph is the object structure or model to be modified (token model),
\tododt{ref}
i.e. the run-time data of the executed software.
The type graph is a set of classes and their relations defining all potential object structures at run-time.
These classes and relatiosn are a so-called type model or meta-model. \tododt{ref}
They are required to specify story patterns.

In summary, a story diagram is a formal kind of UML activity diagram
that embeds story patterns in its activity nodes to precisely describe run-time behavior by means of graph transformations.
