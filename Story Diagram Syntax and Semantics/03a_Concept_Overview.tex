...
To use a simple and familiar notation, story patterns look similar to UML object diagrams.
...
In case of story patterns, the host graph is the object structure or \emph{instance model}\footnote{Thomas K\"{u}hne calls it \emph{token model} \cite{Kue06}.} to be modified,
...
These classes and relations are a so-called \emph{type model} or \emph{metamodel} of the language used to describe instance models \cite{Kue06}.
The type model is required to specify story patterns.

\tododt{nodes, edges become object variables and link variables that are matched to objects and links}
