...
the outgoing activity edges can be provided with the guards \text[success\text] and \text[failure\text] (see Figure~\ref{fig:SD-decisions} a)).
...
Besides \text[success\text] and \text[failure\text] boolean expressions can be used as guards (see Figure~\ref{fig:SD-decisions} b)).
We use the \emph{decision node} -- depicted as a rhomb -- for decisions that do not depend on a previous story node.
...
The decision node can also be used to merge several control flows into one (several activity edges point to a decision node which has only one outgoing activity edge,
see Figure~\ref{fig:SD-loops} b)).
...
For that purpose we use the guards \text[each time\text] and \text[end\text] in combination which is further explained in the following.
A special story node is the \emph{for-each} activity node, depicted by a cascaded activity node (see Figure~\ref{fig:SD-loops} a)).
...
Each for-each activity node has such an outgoing activity edge.

In addition, this edge can be combined with another activity edge having the guard \text[each time\text]
(see Figure~\ref{fig:SD-loops} a)).
...
In this case, there has to be an activity edge leading back to the for-each activity node
to terminate the specified loop by following the \text[end\text] activity edge (see Figure~\ref{fig:SD-loops} a)).

Alternative ways to describe loops in story diagrams are uses of activity edges pointing to a preceding activity node
(see Figure~\ref{fig:SD-loops} b) and c)).
