...
For example, model checking can be used to check if a safety property is always met (\todoall{example!!!}) or if a critical state (\todoall{example!!!}) can ever be reached. \todomvd{Ich finde hier braucht's keine Beispiele. Wir wollen doch nicht Model Checking erklaeren.}
\tododt{Ich wollte hier gerne je ein gutes Bsp. für solche Eigenschaften nennen, damit sich besser was darunter vorstellen kann. Keine Diagramme oder so.}
...
%  \item name our restrictions, e.g., omition of fork and join
  \item \tododt{refer to the appendix due to a complete language reference}
  \item \tododt{mention the grammar for story diagrams (based on grammar in diploma thesis of Thomas Klein \cite{Kle99})?}
\end{itemize}
