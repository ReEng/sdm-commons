...
we use the notation of UML 1.5 activity diagrams, but already use the terminology of the UML 2 and will add constructs from UML 2 like combined fragments to story diagrams in future.}
...
This story diagram takes a graph-based representation (a model of a so-called abstract syntax graph, see Section~\ref{sec:foundations:typedAttrGTS}) of an object-oriented program, e.g., in Java,
...
If for each method specified in a class model there is also a story diagram modeling the method's behavior, the software model can be executed.
In this case, story diagrams specify the behavior of objects whose properties are defined by classes.
For that reason, story diagrams have a \emph{this} variable -- similar to the keyword \emph{this} in Java -- representing an object (class instance) that they belong to (a self reference).
...
Besides execution, the formally defined story diagrams can also be analyzed to guarantee certain behavioral properties \cite{Zue09}.
...
Since story diagrams can be seen as special UML activity diagrams, we reused the class names defined by the UML\footnote{We use the class names from UML 2.}.
...
A modifying story node is also allowed to perform modifications of the matched obejct structure.
For static analyses of graph transformations described with story diagrams
it is helpful to know which tranformations do not modify the instance model (host graph).
...
\begin{figure}[htb]
  ...
  \begin{minipage}[t]{.24\textwidth}
    ...
    %\label{fig:SD-loop-boolean}
  \end{minipage}
  \caption[Examples For Control Flow Simplifications]{Examples For Control Flow Simplifications: case a) becomes b), case c) becomes d)}
...
We use the \emph{junction node} -- depicted as a rhomb -- for decisions that do not depend on a previous story node.
...
we also allow shortcuts as illustrated in Figures~\ref{fig:SD-simplifications} b) and d).
The control flow in case b) is equivalent to that in case a).
The control flows in cases c) and d) are also equivalent to each other.
...
But there is an additional construct to model loops which allows to perform the same operations with each occurrence of a certain object structure.
...
The \text[each time\text] edge is leading to the activity node (or e.g.\ a sequence of such nodes)
...
Each for-each activity node has such an outgoing \text[end\text] activity edge.

\todojr{fresh matches, auch fuer maybe\_bound: Behält ein maybe\_bound-Knoten ein Binding, was erst im Loop gesetzt wurde? Oder zaehlt dafuer nur, was vor dem Loop schon gebunden war? Letzteres erscheint sinnvoller (Stefan fragen wegen Beispiel)}
