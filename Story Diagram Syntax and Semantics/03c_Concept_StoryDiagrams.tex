...
The failure activity edge leads to a final node also labeled with \emph{failure}.
...
Thus, success or failure of a story diagram can be used like an implicitly declared boolean out parameter describing if the operation described by a story diagram is considered to be successful.
...
Note that for story patterns, the success or failure is defined by the language.
...
For whole story diagrams, the interpretation of its success or failure can be specified by the developer.
...
When the execution of a called story diagram ends at a success stop node, then the call is left via the edge labeled with \fe{[success]}.
