...
\subsection{Stop Nodes} \label{sec:StopNodes}
...
In my opinion, the success and failure node are not necessarily dependent on the success or failure of some pattern matchings or transformations.
...
Thus, success and failure of a story diagram is like a boolean out parameter describing if the operation described by a story diagram is considered to be successful.
In which case it is successful is defined by the developer, not the story diagram language.
...
\tododt{I disagree with the propagation in loops.
To enable recursion, I would allow to overwrite the matching that is passed to the for-each activity node.
Consequently, the matching that is passed through the each-time edge is furhter propagated to the following activity nodes
and finally to the for-each node again (following the control flow).
In Figure~\ref{fig:propagation}, \fe{p1} could be overwritten in activity node B or C,
thus, the new matching (not the original one) is passed through the end edge.
The question is if all variables declared in the activity nodes that are reached via the each-time edge
are still visible in the activity node reached via the end edge.
I would say, they are only visible within the loop,
similar to local variables in the scope of a loop in Java which are not visible outside the loop.}
...
If the control flow reaches a stop node, only objects that are contained in the matching which is propagated to the stop node may be returned. The use of decision nodes in combination with activity edges having a boolean guard does not change the propagation of matchings. The boolean condition at the edge only defines where the matching is propagated.
