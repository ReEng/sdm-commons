...
In general, identifying a matching of a story pattern to an instance model includes the \emph{subgraph isomorphism} problem which is \emph{NP-complete}~\cite{Epp95}. 
Story patterns aim to reduce the computational complexity of the matching process by using bound variables.
Therefore, we require at least one bound object variable in each story pattern which is used as a starting point for the matching process.
