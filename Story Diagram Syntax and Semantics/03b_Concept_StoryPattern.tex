...
Since on object set variable does not explicitly represent a collection object (similar to \texttt{java.util.Collection}),
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.9]{figures/InclusionLinks}
...
An inclusion link is represented by a line between an object set variable and another object variable as illustrated in Figure~\ref{fig:InlucionLinks} a).
The circle containing a plus determines which of the two sides of the link is containing the other one.
In Figure~\ref{fig:InlucionLinks} a), the object set \fe{methods} contains an object \fe{m}.
...
But similat to link variables, this relation can be checked to be existent between two matched object variables or be used to match new objects.
In Figure~\ref{fig:InlucionLinks} a), a \fe{Method} object in the object set \fe{methods} is to be matched (found).
...
\create\ or \destroy\ stereotypes (see Figure~\ref{fig:InlucionLinks} b) to e)).
\tododt{explain semantics}
Furthermore, they can also be used between two object set variables as illustrated in Figure~\ref{fig:InlucionLinks} f)
to describe that all objects in the set \fe{selection} are contained in the set \fe{all}.

\tododt{Write the remainder of the section}

\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.9]{figures/InclusionLinksExamples}
  \caption{Exemplary Uses of Inclusion Links}
  \label{fig:InlucionLinksExamples}
\end{figure}

Example in Figure~\ref{fig:reuseObjSet2}

link order constraints with inclusion links?
...
feasible combinations as illustrated in Figures~\ref{fig:InlucionLinkWellFormedness}, \ref{fig:negativeObjects}, \ref{fig:optionalObjects}, and in Table~\ref{tab:bindingCombinations_links}

\todoall{Are objects that are matched to object set variables ordered in the corresponding container? Can such containers contain an object several times?}
