...
\todomcp{If we bind an object set, can we use the bound object in other story pattern? E.g. to insert all elements bound by the object set into a container via a containment link?}
\tododt{Yes, but I would use another concrete syntax (see Figures~\ref{fig:reuseObjSet1}, \ref{fig:InclusionLinksExample1}).}
...
\todoall{Should we add the INDEX constraint again?}
\todoch{We discussed this once and decided to remove it. If we add it, we will have problems with OutOfBoundsExceptions if an element with the specified index in not contained in the list. Why do you need it?}
...
A combination with NEGATIVE binding semantics is also straight-forward. If a link variable is negative and has a \fe{\{first\}} (or \fe{\{last\}}) link position constraint, then the object bound to the target object variable must not be the first (or last) object in the corresponding reference.
...
\todoch{Discuss binding operators: destroy of source link variable, target link variable or both. create of source link variable, target link variable or both. create and delete for direct successor, only delete for indirect successor}
...
\todoch{Discuss binding semantics: optional and negative, optional create only for direct successors}

\todoch{Discuss combination with link position constraints: no unsatisfiable combinations, e.g., link variable has link position constraint first and is target link variable of a link position constraint}
...
\todoch{Again, we need to discuss the semantics here: The proposal by Stephan suggests that in case of a link order constraint originating from or targeting a negative link variable, we negate the link order constraint rather than the link variable. Is that intended and intuitive? Again, we have two options: 1) Negating the link order constraint, i.e., the element is not the direct successor of the other element. That, again, slightly changes the semantics of a negative link. 2) Negating the link variable, i.e., there exists no element which is the direct successor of another element. This would be the same semantics as a non-negated link variable with link position constraint last. I prefer 1) just as Stephan.}
