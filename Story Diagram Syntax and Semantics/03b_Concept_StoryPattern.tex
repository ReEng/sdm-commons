...
\todoall{Also mention that rejecting the match does lead to backtracking to find a new match where this pattern constraint is satisfied. I.e. we go through all isomorphic matches to find one that satisfies the pattern constraint. But in case of the example in Figure~\ref{fig:patternConstraint} there is no backtracking due to the collection variable (there is no other match and we do not try to match subsets of the methods in the class).}
