...
containment in a set of objects, i.e.\ inclusion link from object set variable to any object (set) variable means that the object set contains the objects.

inclusion links are not typed

inclusion links usable as illustrated in Figure~\ref{fig:InlucionLinks}

Example in Figure~\ref{fig:reuseObjSet2}

object set contains an object;
object set contains another set of objects?;
negative, optional, mandatory inclusion links;
create and destroy with object inclusion links;
link order constraints with inclusion links?


\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.8]{figures/InclusionLinks}
  \caption{Examples of inclusion links}
