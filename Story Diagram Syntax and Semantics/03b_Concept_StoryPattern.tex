...
Story patterns are typed attributed graph transformation rules with inheritance on node types (cf. Section \ref{sec:foundations:typedAttrGTS}) that may be embedded into an activity node of a story diagram (cf. Section \ref{sec:StoryDiagrams}).
 By using a type model as introduced in Section \ref{sec:typeGraph}, story patterns enable inheritance and polymorphism for matching nodes and edges.
This allows for specifying graph replacement rules for object-oriented models.

The nodes and edges in a story pattern are called object variables and link variables, respectively. 
As for normal graph transformations, object and link variables are matched to the nodes and edges of the host graph. 
In contrast to typed attributed graph transformation, story patterns explicitly require to use isomorphic matchings, i.e., two object variables of a story pattern may not be matched to the same node of the host graph.
...
Nodes and edges being created (or deleted) are annotated with \create (or  \destroy, respectively). 
Nodes are are preserved by the graph transformation carry no annotation.
Then, the LHS consists of all nodes that are preserved and deleted, while the RHS consists of all nodes that are preserved and created. 
The deletion of nodes follows the single pushout approach.
...
In the example, the nodes \fe{parentClass}, \fe{oldMethod}, and \fe{newMethod} are bound variables, i.e., they already refer to objects of the host graph (cf. Section \ref{sec:StoryPatterns:binding:states}).
The nodes \fe{anyMethod} and \fe{call} are unbound. 
When applying the story pattern, first a match for \fe{anyMethod} and \fe{call} is searched. 
A possible match will be any method in \fe{parentClass} which contains a call to \fe{oldMethod}. 
If the matching was successful, the link from \fe{c} to \fe{oldMethod} will be deleted and the link from \fe{c} to \fe{newMethod} will be created.

In our concrete syntax, the nodes which are not changed by the story pattern are visualized in black. 
...
An unbound node is labeled by its name and the name of the corresponding type. 
For bound nodes, we omit the name of the type.
...
The nodes of our host graph carry attributes. 
...
We use \emph{attribute constraints} to restrict the matching of object variables to nodes of the host graph that have specific attribute values. 
Thus, attribute constraints are considered to be part of the LHS and do not change the host graph. 
...
In the example, we match a method being contained by the class \fe{theClass}. 
...
When using attribute assignments, the value of the attribute is not considered while matching the LHS to the host graph. 
