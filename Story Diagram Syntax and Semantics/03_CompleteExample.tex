...
\todomvd{Explain the situation (violated interface), illustrate it with a class diagram. Explain that this violates the ``Program to an interface\ldots'' principle from \cite{GHJV95}. Explain the possible remedy (declaring the accessed method in the interface). Show the consequences (all implementing classes have to be adapted). Point out the similarity of this refactoring to Fowler's ``Extract Interface'' refactoring \cite{Fow99}.}

\section{Story diagram of the example}

\todomvd{Adapt the story diagram from Marie's thesis.}