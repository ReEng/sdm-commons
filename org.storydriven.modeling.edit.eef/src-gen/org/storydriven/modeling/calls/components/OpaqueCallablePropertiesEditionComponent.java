...
public class OpaqueCallablePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (CallsViewsRepository.OpaqueCallable.Properties.callExpression == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				MethodCallExpression eObject = ExpressionsFactory.eINSTANCE.createMethodCallExpression();
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (SDMPackage.eINSTANCE.getCommentableElement_Comment().equals(msg.getFeature()) && basePart != null && isAccessible(CallsViewsRepository.OpaqueCallable.Properties.comment)) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (CallsViewsRepository.OpaqueCallable.Properties.name == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(CallsPackage.eINSTANCE.getOpaqueCallable_Name().getEAttributeType(), (String)newValue);
					}
